...
研究室紹介の動画:
[小出研究室紹介ビデオ 2026-01-20](https://youtu.be/SszCCza6PEM)
...
**文字列の右端の次にある空白のマスに `Y` または `N` を書いて停止する**ものとする。
（ヒント: 状態は「これまでに読んだ 1 が偶数個」「奇数個」の2つで足りる。
表の形は 1.2 の「1を足す機械」と同じでよい。）

**解答形式**: 1行に1つの規則を、次の形で書け。
`(偶数, 0) → 0 を書く, 右へ, 偶数`
