...
- 量子計算機が計算量のクラスをどう変え、何を変えないかを説明できる
...
第1回で「量子計算機は計算できる**範囲**を変えないが、**速さ**を変える」と述べた。
計算量の言葉が揃ったので、ここで正確に言い直す。

古典的な計算機で多項式時間で解ける問題のクラスが **P** だった。
量子計算機で多項式時間で解けるクラスを **BQP** という。

- **P ⊆ BQP** — 量子計算機でできることは、古典的な計算機でも（遅いが）できる。
  だから第1回の「計算できる範囲は広がらない」が成り立つ
- **BQP に入るが、P に入るとは知られていない問題がある** — その代表が
  **素因数分解**である。Shor のアルゴリズムは、これを量子計算機で多項式時間で解く

**ここが最も誤解されるところである。**

> 「量子計算機があれば NP完全問題も解ける」——これは誤りである。

量子計算機が NP完全問題を多項式時間で解けるとは考えられていない。
しらみつぶし探索を速くする **Grover のアルゴリズム**は、
n 通りの探索を √n 回に減らす。しかし指数時間の問題では n が 2ᵏ なので、
**2ᵏ が 2^(k/2) になるだけ**である。指数時間のままである。

共通鍵暗号の鍵を1ビット伸ばせば元の強度に戻る、という程度の加速にすぎない。

### 素因数分解は、どこにいるのか

では Shor はなぜ効くのか。**素因数分解が NP完全ではない**からである。

- 素因数分解は NP に属する（答えを与えられれば、掛け算して確かめられる）
- しかし NP完全とは考えられていない。P と NP完全の**中間**にあると見られている
- 量子計算機が効くと分かっているのは、この中間の領域である

つまり量子計算機の強みは「難しい問題を何でも速く解く」ことではなく、
**特定の構造を持つ問題に刺さる**ことである。
素因数分解と離散対数には「周期を見つける」という構造があり、Shor はそこを突く。

**第12回への伏線**: 現在の公開鍵暗号（RSA、楕円曲線暗号）は、
まさにこの中間の問題に安全性を託している。だから Shor で破れる。
移行先には、量子計算機でも効率的に解けそうにない別の問題
（格子問題など）が選ばれている。
...
└── 計算可能
    ├── 指数時間しか知られていない
    │    ├── NP完全（SAT、巡回セールスマン）      量子計算機でも指数時間のまま
    │    └── 中間（素因数分解、離散対数）         量子計算機なら多項式時間 → 第12回
    └── 多項式時間で解ける（P ⊆ BQP）
```
...
**2-5.** 次の4つのうち、NP完全と知られているものは **(b) の数独**（n×n に
一般化したもの）である。残る3つが NP完全でない理由を、それぞれ
「入力の大きさ n に対してどのくらいの時間で解けるか」に触れて述べよ。

(a) 配列のソート (c) 二分探索
(d) グラフの最短経路（ダイクストラ法で解けるもの）

**2-6.** 「量子計算機が実用化されれば、巡回セールスマン問題も現実的な時間で
解けるようになる」という主張は正しいか。Grover のアルゴリズムがどの程度の
加速をもたらすかに触れて答えよ。

**2-7.** あなたが使っているサービスで「厳密な最適解ではなく、近似解を返している」
...
- 量子計算機は P ⊆ BQP なので計算できる範囲を変えない。NP完全問題も
  指数時間のままである。効くのは素因数分解のような「中間」の問題で、
  そこに現在の公開鍵暗号が乗っている（第12回）
