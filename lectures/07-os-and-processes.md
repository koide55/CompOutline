...
**7-7.** 共有変数 `count`（初期値 10）を、2つのスレッド A, B がそれぞれ
5回ずつインクリメントする。排他制御はない。
インクリメントが「**読む → 1 足す → 書き戻す**」の3段階に分かれることを
踏まえて答えよ（7.4 の表を参照）。

(a) すべて正しく反映された場合、最終値はいくつか。

(b) 次の実行順序について、**時刻5のあと**と**時刻6のあと**の `count` の値を
    それぞれ答えよ。また、A と B が1回ずつインクリメントしたのに、
    `count` はいくつ増えたかを答えよ。
...
(c) (b) と同じ食い違いが5回とも起きたとすると、最終値はいくつになるか。

（なお実際には、片方が古い値をさらに長く持ち続けることで 12 まで下がりうる。
ここでは (b) の形が繰り返される場合だけを考えればよい。）

**7-8.** デッドロックの4条件を挙げ、それぞれを崩す具体的な方法を1つずつ述べよ。

**7-9.** 残高 1000 の口座に対して、2つのスレッドが同時に `withdraw(600)` を
呼んだところ、**両方が `True` を返し、残高が −200 になった。**
どの行の直後に別のスレッドが割り込むとこうなるかを示し、修正方針を述べよ。
