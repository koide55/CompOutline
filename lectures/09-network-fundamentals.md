...
| 5–15 | 9.1 データの届け方 |
| 15–28 | 9.2 階層モデル |
| 28–39 | 9.3 インターネットの構造 |
| 39–64 | 9.4 物理層とデータリンク層 |
| 64–85 | 9.5 インターネット層：IP |
| 85–90 | 演習・まとめ |
...
9.4 で「IP アドレスは住所、MAC アドレスは指紋」と述べたのは、
この階層性のことである。
...
![図 9-6 パケットの中継](../figures/fig-09-06.svg)
...
![図 9-7 NAPT による変換](../figures/fig-09-07.svg)
...
**9-8.** `198.51.100.130/26` について、
(a) ネットワークアドレス (b) ブロードキャストアドレス
(c) 機器に割り当てられるアドレスの個数 を求めよ。

**9-9.** 次の経路表を持つルータが、宛先 `192.0.2.130` のパケットを
...
- IP アドレスは階層構造を持ち、それによって経路表を小さく保てる。
  ルータは最長一致で経路を選ぶ
- IPv4 は枯渇し、NAT で凌いでいる。NAT は外から内への接続を塞ぐので、
  対等な通信を難しくした。根本的な解決は IPv6 である

**次回**: 届ける道はついた。
では、その道の上で、名前しか知らない相手とどうやってやりとりするのか。
失われるかもしれない経路で、どうやって確実に、そして安全に。
