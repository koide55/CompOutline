...
- バージョン管理が何を解決するかを説明できる
- テストがバグの不在を証明できない理由を、第1回の決定不能性から説明できる
...
| --- | --- |
| 0–5 | 前回の復習 |
| 5–18 | 4.1 機械語とアセンブリ言語 |
| 18–34 | 4.2 高級言語と処理系 |
| 34–52 | 4.3 実行ファイルができるまで |
| 52–69 | 4.4 言語の分類と選び方 |
| 69–85 | 4.5 書いたあと、動くと確かめるまで |
| 85–90 | 演習・まとめ |
...
ここまでは「ソースコードが機械語になる」道筋を追った。
現場ではその周りに、もう1つの循環がある。**書く → 記録する → 確かめる**である。
処理系の話と違って、こちらは**人間の側の仕組み**である。

### バージョン管理

ソースコードは1回書いて終わりではない。直し、戻し、複数人で同時に触る。
**バージョン管理システム**がその履歴を扱う。

| 何ができるか | なぜ要るか |
| --- | --- |
| 変更の履歴を残す | 「なぜこう書いたか」が後から追える |
| 任意の時点に戻せる | 壊したときに引き返せる |
| 複数人の変更を統合できる | 同じファイルを同時に触っても失われない |
| 分岐して試せる | 本流を壊さずに実験できる |

記録の単位は**コミット**である。「この変更のまとまり」を1つとして残す。
第11回のトランザクションと同じで、**中途半端な状態を記録しない**のが要点である。

現在の主流は **git** で、**分散型**である。
各人が履歴の完全な複製を持ち、中央のサーバが落ちても作業が続く。
第9回のパケット交換と同じ発想——中央に依存しない構造である。

**「いつ壊れたか」を二分探索で特定できる。**
いま動かないが、1か月前は動いていた。その間にコミットが100個ある。
真ん中のコミットで試し、動けば後半に、動かなければ前半に原因がある。
**7回で1個に絞れる**（第2回の二分探索。git では `git bisect` が自動化する）。

### テスト

**テスト**は、プログラムが期待どおり動くかを確かめる作業である。

| 種類 | 何を確かめるか |
| --- | --- |
| 単体テスト | 関数や部品が個別に正しく動くか |
| 結合テスト | 部品を組み合わせたときに正しく動くか |
| 回帰テスト | **以前直したものが、また壊れていないか** |

**自動化がいちばん効くのは回帰テストである。**
人間は一度直した箇所を二度と確かめない。機械は毎回確かめる。

### ただしテストは、バグが無いことを証明しない

第1回で述べたとおりである。

> ライスの定理により、プログラムの入出力の振る舞いに関する
> 自明でない性質は、すべて決定不能である。

テストは有限個の入力で試すだけなので、**試していない入力については何も言えない**。
「テストが通った」は「バグが見つからなかった」であって、「バグが無い」ではない。

> **テストはバグの存在を示せるが、不在は示せない。**

**そして第1回の3つの迂回策が、そのまま開発の道具立てになる。**

| 第1回の迂回策 | 対応する道具 | 何をしているか |
| --- | --- | --- |
| **近似する** | テスト、静的解析 | 有限個の入力で試す。危ない箇所を過剰に報告する |
| **対象を狭める** | 型システム（4.4） | 決定可能な性質だけを、機械が全入力について保証する |
| **人間を入れる** | コードレビュー | 機械に判定できないことを人が見る |

**型検査とテストは競合しない。** 型は「ある種の誤りが無いこと」を
すべての入力について保証する。テストは「ある入力で正しく動くこと」を示す。
**保証の種類が違うので、両方使う。**

### 継続的インテグレーション

コミットするたびに、自動でビルドしてテストを走らせる仕組みを
**継続的インテグレーション**（CI）という。

狙いは1つである。**壊れたことを早く知る。**

壊れてから気づくまでが長いほど、原因の候補が増える。
コミットごとに確かめれば候補は常に1個なので、
**二分探索する必要すらなくなる。**
...
**解答形式**: 観点ごとに1行で、次の形で書け。
`(a) 実行速度: コンパイラ方式は… / インタプリタ方式は…`
...
**4-6.** 「テストが全部通ったので、このプログラムにバグは無い」という主張に
反論せよ。第1回の内容に触れること。

**4-7.** 1か月前は動いていたプログラムが、いま動かない。
その間にコミットが100個ある。原因のコミットを特定するのに、
最小で何回試せばよいか。またその方法を述べよ。

**4-8.** 静的型付けの言語を使っていれば、テストは要らないか。
型検査とテストが「何を保証するか」の違いに触れて答えよ。

**4-9.** あなたが今後1年で1つ言語を学ぶとしたら何を選ぶか。
...
- 言語は 実行方式・型・パラダイム・メモリ管理 の軸で位置づける。
  選択は生態系・性能要件・保守性で決まる
- バージョン管理は履歴と分岐を扱う。コミットは「中途半端な状態を記録しない」単位。
  「いつ壊れたか」は二分探索で特定できる（第2回）
- テストはバグの存在を示せるが、不在は示せない（第1回のライスの定理）。
  第1回の3つの迂回策が、テスト・型システム・レビューに対応している
