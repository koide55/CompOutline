...
**投機実行**——予測に基づいて先に実行してしまう——は、
2018年に発見された Spectre / Meltdown 脆弱性の原因になった。
外れた予測は取り消されるが、**キャッシュに残った痕跡は消えない**。
これを観測すると、本来読めないはずのデータが推測できてしまう。
性能のための工夫が安全性の穴になった例である（第12回で再訪）。
...
![図 5-6 アムダールの法則](../figures/fig-05-06.svg)
...
**5-6.** 「CPU のコア数を2倍にすれば処理時間は半分になる」という主張は
